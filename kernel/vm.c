...
// just follow the kvmmap to add a mapping to the user page table
...
void procFreeKernelpt(pagetable_t kernelpt) {
    for (int i = 0;i < 512;i++) {
        pte_t pte = kernelpt[i];
        //如果pte是有效的
        if ((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0) {
                uint64 child = PTE2PA(pte);
                procFreeKernelpt((pagetable_t)child);
                kernelpt[i] = 0;
        }
        kernelpt[i] = 0;
    }
...
void freeprockvm(struct proc* p) {
  pagetable_t kpagetable = p->kernelpt;
...
void vmprinter_helper(pagetable_t pagetable, int depth) {
    static char *indent[] = {
        "",
        "..",
        ".. ..",
    ...
    for (int i = 0; i < 512; i++) {
        ...
        if (pte & PTE_V) {
            ...
            if ((pte & (PTE_R | PTE_W | PTE_X)) == 0) {
                uint64 child = PTE2PA(pte);
                vmprinter_helper((pagetable_t)child, depth + 1);
            }
        }
    }
}
...
void vmprint(pagetable_t pagetable) {
    printf("page table %p\n", pagetable);
    vmprinter_helper(pagetable, 1);
}

//follow mappages函数，不过不需要再panic remapping强制复写即可 
int umappages(pagetable_t pagetable, uint64 va, uint64 pa, uint64 size, int perm) {
    uint64 a, last;
    pte_t *pte;
...
//将从begin到end的虚拟地址的映射从oldpage拷贝到newpage
int pagecopy(pagetable_t oldpage, pagetable_t newpage, uint64 begin, uint64 end) {
    ...
    for (i = begin;i < end;i += PGSIZE) {
        ...
        //pte不合法
        if ((*pte & PTE_V) == 0) {
            panic("pagecopy oldpage invalid");
        }
        pa = PTE2PA(*pte);
        flags = PTE_FLAGS(*pte) & (~PTE_U);
        if (umappages(newpage, i, pa, PGSIZE, flags) != 0) {
            goto err;
        }
    }
    return 0;
